...
'''
read  csv file to save all col names key is col name, value is index, which will be used in other functions
'''
...
'''
def create_gentrification_table(table_name):
...
'''
def insert_from_csv_to_sql_zillow(csv_path = zillow_path, table_name = 'Zillow', cols = cols_opt, cols_all = cols_all, year_bar = 2005, db_path = zillow_db_path):
...
def insert_from_csv_to_sql_gentrification(csv_path = gentrification_path, table_name = 'Gentrification', db_path = gentrification_db_path):
    conn = sqlite3.connect(db_path)
    c = conn.cursor()
    print("start insert data")
    gentri_file = open(csv_path, "r")
    reader = csv.reader(gentri_file)
    id = 0
    next(reader, None)
    for row in reader:
        try:
            ...
            year = row[1]
            zipcode = str(row[2]) 
            for i in range(5 - len(str(row[2]))):
                zipcode += "0"
...
if __name__ == "__main__":
    ...
    '''
    cols = list(set(read_csv_header(zillow_path).keys()) - set(['Date', 'RegionName']))
    print(cols)
    create_zillow_table('Zillow', cols_opt = cols)
    insert_from_csv_to_sql_zillow(zillow_path, 'Zillow', cols = cols)

    #create_gentrification_table('Gentrification')
    #insert_from_csv_to_sql_gentrification(gentrification_path, 'Gentrification', gentrification_db_path)